...
ax.set(xlabel="Threshold", ylabel=f"Micro F1 Score", ylim=(0.30, 0.65))
ax.grid(visible=True, which="minor", color="black", linewidth=0.075)
sns.move_legend(ax, "upper right")
plt.plot([0.5, 0.5], [0, 1], color="grey", linestyle="--")
ax.get_figure().savefig(
    f"files/images/mimiciv_icd10/threshold_tuning_micro_full.png",
    bbox_inches="tight",
    dpi=400,
)
plt.clf()
...
ax.set(xlabel="Decision Boundary", ylabel=f"Macro F1 Score", ylim=(0, 0.35))
ax.grid(visible=True, which="minor", color="black", linewidth=0.075)
sns.move_legend(ax, "upper right")
plt.plot([0.5, 0.5], [0, 1], color="grey", linestyle="--")
ax.get_figure().savefig(
    f"files/images/mimiciv_icd10/threshold_tuning_macro_full.png",
    bbox_inches="tight",
    dpi=400,
)
